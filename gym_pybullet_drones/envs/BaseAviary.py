...
class BaseAviary(gym.Env):
    ...
    def _exportImage(self,
        ...
        if img_type != ImageType.RGB:
            ...
            # Lấy hình ảnh từ camera của drone (ví dụ: drone thứ nhất)
            rgb_image, depth_image, segmentation_image = self._getDroneImages(nth_drone=0)

            # Đường dẫn để lưu trữ hình ảnh
            output_path = "home/khoa/Cam_drones"

            # Lưu trữ hình ảnh màu (RGB)
            self._exportImage(ImageType.RGB, rgb_image, output_path, frame_num=1)

            # Lưu trữ hình ảnh depth
            self._exportImage(ImageType.DEP, depth_image, output_path, frame_num=1)

            # Lưu trữ hình ảnh segmentation
            self._exportImage(ImageType.SEG, segmentation_image, output_path, frame_num=1)
    ...
    def _addObstacles(self):
        ...
        p.loadURDF("/home/thanhkhoa/anaconda3/lib/python3.11/site-packages/pybullet_data/table/table.urdf",
                   [0, 2, .0],
                   p.getQuaternionFromEuler([0,0,0]),
                   physicsClientId=self.CLIENT
                   )
