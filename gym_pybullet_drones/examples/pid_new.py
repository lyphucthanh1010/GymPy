...
def run(
    ...
    for i in range(0, int(duration_sec*env.CTRL_FREQ)):
        ...
        print(obs[0])
        object_position = obs[:3]
        
        if object_position[0][0] >5:
            action[0,0] = 0.5
        else:
            action[0,0] = -0.5
        action_from_model, _= model.predict(obs, deterministic= True)
        action[0] =  action_from_model[0]
